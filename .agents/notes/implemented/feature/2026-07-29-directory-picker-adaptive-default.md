...
A third sibling package, **`dsh-host-directory-picker-auto`**: a node-half-only *chooser* that owns no picking code and no UI. Its `apply` samples the host facts exactly once at boot — declared remote-browser access, bind host from the injected `webServer`, `SSH_CONNECTION`/`SSH_TTY`, platform, `DISPLAY`/`WAYLAND_DISPLAY`, and a `PATH` probe for a Linux chooser binary (zenity/kdialog) — resolves them through one exported pure function, and mounts the chosen dual-face backend with `ctx.loader.create({name})` into the Loader's **in-memory root tree**; the effect's disposer removes the entry and joins the backend fiber's teardown, so unloading the chooser settles only after the backend quiesces. `native` requires every attended-and-servable signal: no declared remote browser ∧ loopback bind ∧ no SSH markers ∧ a display session the native backend can drive — assumed on darwin/win32, requiring `DISPLAY`/`WAYLAND_DISPLAY` plus a chooser binary on linux, and never true elsewhere. Anything ambiguous resolves to `browse`, which works everywhere. The Web bundle sets `remoteBrowserAccess` when it has a public origin or `--trusted-host`, covering a private loopback hop behind a reverse proxy or tunnel. Composing `-native` or `-browse` directly remains the pin.
...
- **Infer remote reachability only from the server bind address.** Rejected because reverse proxies and tunnels normally retain a loopback-only private hop while accepting browsers through another authority. The explicit deployment fact preserves the generic chooser and lets the Web bundle derive it from configuration it already owns.
...
- The shipped web GUI adapts out of the box: attended local host → OS chooser; a declared public authority, SSH launch, all-interfaces bind, headless host, unsupported platform, or Linux without a chooser binary → in-app browser. Detection still depends on declared reachability: a detached tmux session loses `SSH_*`, a non-Aqua darwin process still counts as displayed, and an undeclared `ssh -L` route looks loopback-local. A wrong `native` choice degrades to the backend's existing retryable failure dialog; these deployments set `remoteBrowserAccess` or compose `-browse` directly.
