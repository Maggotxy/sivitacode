...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default. The compatibility `dsh` entry rejects `--host 0.0.0.0`. The SivitaCode entry permits it only with Web authentication, a canonical public HTTPS origin, and trusted reverse-proxy CIDRs; its readiness line and agent runtime then use that public origin. `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence.
