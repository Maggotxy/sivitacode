...
[`dsh-access-control`](../../packages/identity/access-control/README.md) 持有持久本地账户、Argon2id 凭据、服务端会话、请求局部 actor、全局角色权限上限、授权决策与安全审计记录。传输认证绑定已验证 actor；Host 消费者在分发前执行全局权限上限，并可用资源级授权进一步收窄权限。

下方公共类型和服务方法由所属包生成。浏览器载荷不携带可信 `AccessActor` 或角色值。
