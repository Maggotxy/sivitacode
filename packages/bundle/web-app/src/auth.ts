...
function loginPage(error = ''): string {
  ...
  // Keep authentication white-led and calm; brand color belongs on identity and focus, not the whole page.
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#f7f8fa"><title>SivitaCode 登录</title><style>body{font-family:system-ui,sans-serif;background:linear-gradient(180deg,#fff 0,#f7f8fa 100%);color:#19171f;display:grid;place-items:center;min-height:100vh;margin:0}main{width:min(24rem,calc(100% - 2rem));background:#fff;border:1px solid #e4e4e7;border-radius:18px;padding:2rem;box-sizing:border-box;box-shadow:0 22px 60px #18181b12}.brand{display:flex;align-items:center;gap:.75rem}.mark{width:2rem;height:2rem;color:#7c3aed}.mark path:last-child{stroke:#0891b2}h1{font-size:1.35rem;margin:0}.tagline{color:#71717a;font-size:.88rem;margin:.55rem 0 1.75rem}label,input,button{display:block;width:100%;box-sizing:border-box}label{color:#52525b;font-size:.9rem}label+label{margin-top:1rem}input,button{font:inherit;padding:.8rem;border-radius:9px;margin-top:.5rem}input{background:#fff;color:inherit;border:1px solid #d4d4d8;outline:none}input:focus{border-color:#8b5cf6;box-shadow:0 0 0 3px #8b5cf61f}button{margin-top:1.35rem;background:#242129;color:#fff;border:0;font-weight:650;cursor:pointer}button:hover{background:#17151b}p{color:#c2415b}</style></head><body><main><div class="brand"><svg class="mark" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 2.75 20.25 7.5v9L12 21.25 3.75 16.5v-9L12 2.75Z" stroke="currentColor" stroke-width="1.8"/><path d="M15.8 8.2H10a2 2 0 0 0 0 4h4a2 2 0 0 1 0 4H8.2" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg><h1>SivitaCode</h1></div><div class="tagline">Web-first coding agent</div>${message}<form method="post" action="${LOGIN_PATH}"><label>用户名<input name="username" autocomplete="username" required autofocus></label><label>密码<input name="password" type="password" autocomplete="current-password" required></label><button type="submit">登录</button></form></main></body></html>`
}
