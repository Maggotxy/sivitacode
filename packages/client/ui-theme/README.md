...
The built-in SivitaCode identity uses the `--dsw-static-sivita-*` violet scale for reasoning and primary actions and the `--dsw-static-sivita-cyan-*` scale for execution signals. Feature packages consume only the corresponding semantic aliases; provider colors never define the product palette.
...
- **The token sheets are the sole color authority** — feature packages use the nearest semantic token. Design-owner-approved additions enter as a static scale plus a semantic alias in the same change (`--dsw-static-sivita-600` / `--dsw-alias-brand-primary`).
