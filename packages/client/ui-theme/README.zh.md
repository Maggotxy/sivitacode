...
内置 SivitaCode 身份以 `--dsw-static-sivita-*` 紫罗兰色阶表示推理和主要操作，以 `--dsw-static-sivita-cyan-*` 色阶表示执行信号。功能包只能使用对应的语义别名；模型提供方颜色不会定义产品配色。
...
- **token 样式表是颜色值的唯一权威来源**：功能包使用最接近的语义 token。设计负责人批准的新增值必须在同一变更中以静态色阶和语义别名的形式进入（`--dsw-static-sivita-600` / `--dsw-alias-brand-primary`）。
