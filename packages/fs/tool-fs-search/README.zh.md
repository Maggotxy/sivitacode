...
控制平面与本机目标调用使用随包交付、覆盖受支持平台（macOS/Linux/Windows，x64/arm64）的二进制，因此宿主无需安装 `rg`。持久 SSH 或 OCI 目标会改为通过该 Agent 的 subprocess provider 解析 `rg`，并在所选执行世界内运行；远端主机或容器镜像因此必须提供 `rg`。返回路径以会话工作区为基准，并可由路由文件系统继续读取，因为 Inventory 会为同一目标和工作区挂载文件系统与 subprocess provider。
...
- **自定义非 Inventory provider 仍自行负责共置**——Inventory 会把文件系统和 subprocess 路由到同一目标工作区；独立组合的 provider 对必须自行保留这一关系。
- **远端目标的工具链必须包含 `rg`**——打包的 `@vscode/ripgrep` 二进制服务于控制平面与本机目标调用；SSH 主机和 OCI 镜像解析各自目标本地的可执行文件，缺失时以 `SEARCH_FAILED` 失败，绝不回到控制平面运行搜索。
