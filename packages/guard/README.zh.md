...
guard 插件拒绝无效运行时组合、监视 agent loop（智能体循环）中的无效模式，并强制执行单次调用预算。guard 是服务和扩展点的自包含消费方，而非可替换能力。
...
| [`execution-world-coherence/`](execution-world-coherence/README.md) | 拒绝来自不同执行环境的文件系统与子进程提供方 | 注入 `ctx.fs` 与 `ctx.subprocess` |
