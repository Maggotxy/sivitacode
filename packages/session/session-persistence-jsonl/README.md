...
- **Deletion is exact, retention is manual** — `delete()` validates the stored header, removes only that session-owned directory, and fsyncs its parent on POSIX; age/size pruning and archival remain deployment policy.
