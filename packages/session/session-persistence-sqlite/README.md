...
- **Deletion is exact, retention is manual** — `delete()` removes one inactive session row atomically and `ON DELETE CASCADE` removes only its events; age/size pruning and archival remain deployment policy.
