...
| `supportsDeletion: boolean` | 明确说明后端是否支持永久删除非活动会话。 |
...
| `delete(id, signal?): Promise<boolean>` | 永久移除一个非活动、已实体化会话，并报告它是否存在。基于协调器的实现会与写入和 retirement 串行化，拒绝实时或已预留身份，并且只在破坏性后端工作开始前接受取消。不支持的后端会明确拒绝。 |
...
| `deleteStored?(id, signal?)` | 在协调器完成生命周期检查后永久移除一个精确的已实体化身份。缺失时返回 `false`；省略该 hook 表示不支持删除。 |
...
- **没有自动保留策略**：已经支持精确删除，但按年龄／容量配额、法律保留、归档和定时剪枝仍属于部署工作。
