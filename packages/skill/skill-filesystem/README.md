...
The first-party filesystem `write` and `edit` tools also synchronously invalidate the provider through `fs/observed` when their target could affect a watched skill entry. This fast path makes the next model step observe its own filesystem mutation without waiting for the host watcher. External IDE, Git, shell, and process changes on the local execution world rely on Chokidar or the missing-path probe. A non-local `ctx.fs` cannot be observed by host filesystem APIs, so target-backed roots are rescanned on every lookup and remain complete but uncacheable; trusted bundled host roots retain their local watcher. Existing-root watchers remain persistent until effect teardown so Chokidar owns asynchronous native error events; startup/runtime watcher failures are logged and retried. Discovery still scans readable roots and returns their candidates for direct loading, but a watcher failure marks the observation incomplete. Effect teardown closes every watcher and contains late callbacks.
